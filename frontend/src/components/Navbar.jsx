    ...
    return (
        <header className='w-full bg-transparent top-0 left-0 right-0 transition-all ease-in duration-300'>
            <nav className={`py-4 lg:px-24 px-4 ${isSticky ? "sticky top-0 left-0 right-0 bg-blue-300" : ""}`}>
                ...
                <div className={`space-y-4 px-4 mt-12 py-7 bg-blue-700 text-white ${isMenuOpen ? "block fixed top-0 right-0 left-0" : "hidden"}`}>
                    {
                        navItem.map(({link , path}) => <Link key={path} to={path} className='block text-base uppercase cursor-pointer hover:text-blue-700'>{link}</Link>)
                    }
                </div>
            </nav>
        </header>
